...
void UInventoryComponent::UpdateInteractionWidget() const
{
...
FItemAddResult UInventoryComponent::HandleNonStackableItems(UItemBase* InputItem)
{
	...
	{
		return FItemAddResult::AddedNone(FText::Format(
			FText::FromString("Could not add {0} {1}to the inventory. All inventory slots are full."), InputItem->TextData.Name));
	}
...
int32 UInventoryComponent::HandleStackableItems(UItemBase* ItemIn, int32 RequestedAddAmount)
{
	...
	while (ExistingItemStack)
	{
		...
		else if (WeightLimitAddAmount <= 0)
		{
			...
			{
				// this block will be reached if distributing an item across multiple stacks and the weight limit is hit
				OnInventoryUpdated.Broadcast();
				return RequestedAddAmount - AmountToDistribute;
			}
		...
		// check if there is still another valid partial stack of that item
